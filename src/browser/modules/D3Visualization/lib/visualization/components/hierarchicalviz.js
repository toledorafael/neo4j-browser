// https://observablehq.com/@d3/hierarchical-edge-bundling@417
export default function define (runtime, observer) {
  const main = runtime.module()
  const fileAttachments = new Map([['fileInteractions.json', new URL('./files/fileInteractions.json')]])
  ...
  main.variable(observer('data')).define('data', ['hierarchy', 'FileAttachment'], async function (hierarchy, FileAttachment) {
    return (
      hierarchy(await FileAttachment('fileInteractions.json').json())
    )
  })
  ...
  main.variable(observer('bilink')).define('bilink', ['id'], function (id) {
    return (
      function bilink (root) {
        ...
        for (const d of root.leaves()) d.incoming = [], d.outgoing = d.data.interacts.map(i => [d, map.get(i)])
